...
def pyramid_sliding_window_detection(net, image, scale, winW, winH, stepSize,device):
    ...
    for resized in pyramid(image, scale=scale):
        ...
        for (x, y, window) in sliding_window(resized, stepSize=stepSize, windowSize=(winW, winH)):
            ...
            # Added support to cuda (if available)
            resized_tensor = torch.from_numpy(window).float().to(device)
